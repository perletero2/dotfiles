...
if [[ ! -d "$dir" ]] ; then
  mkdir -p "$dir" || {echo "Failed to create Dotfiles directory" exit1}
fi
...
updateRepo() {
  ...
  if [ "$(git rev-parse --is-inside-work-tree 2>/dev/null)" != "true" ] ; then # Check if current dir is a git repo
      echo "Cloning repo..."
      ...
      echo "Updating repo..."
      git fetch --all && git stash || {echo "Failed to rebase updates from the remote repo" exit 1}
      ...
      git remote set-url origin "$remote_url" || {echo "Failed to set remote url for $dir" exit 1}
      git fetch --all && git stash || {echo "Failed to fetch from source" exit 1}
      git rebase origin/main || {echo "Failed to rebase updates from the remote repo" exit 1}
      git stash pop || {echo "Failed to apply stashed changes" exit 1}
  fi
}
